...
class InputHandle:
    ...
    def begin(self, do_shuffle = True):
        self.indices = np.arange(self.total(),dtype="int32") #生成索引，有多少组数据生成多少组索引
        if do_shuffle:
        ...
        self.current_batch_indices = self.indices[
            self.current_position:self.current_position + self.current_batch_size]#得到目前batch的索引
    ...
    def input_batch(self):
        ...
        input_batch = np.zeros(
            (self.current_batch_size, self.seq_length,32,3)).astype(self.input_data_type)
        for i in range(self.current_batch_size):
            batch_ind = self.current_batch_indices[i]
            data_slice = self.data[batch_ind]
            data_slice = data_slice[0:self.seq_length]
            input_batch[i, :self.seq_length, :] = data_slice  # 把这一batch的第i项从data中找到并填上
